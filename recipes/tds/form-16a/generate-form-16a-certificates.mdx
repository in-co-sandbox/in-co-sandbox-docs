...
<Steps>
  ...
  <Step title="Submit a Form 16A job" stepNumber={2} titleSize="h2">
    Submit a Form 16A generation job using the [**Submit TDS certificate job**](/api-reference/tds/compliance/certificates/endpoints/submit_job) endpoint. Set the path to `form16a` and the `form` field to `26Q` (or `27Q` for non-resident payments).
  ...
  <Step title="Poll and fetch certificates" stepNumber={3} titleSize="h2">
    ...
    Use the [**Get TDS certificate job status**](/api-reference/tds/compliance/certificates/endpoints/poll_job) endpoint with the `job_id` from Step 2:
    ...
    To list jobs for a TAN and period, use the [**Search TDS certificate jobs**](/api-reference/tds/compliance/certificates/endpoints/fetch_job) endpoint:
