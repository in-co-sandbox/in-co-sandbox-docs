...
---
Sandbox API requests will take JSON payloads, query, and path parameters and provide standard JSON responses. All `GET` and `DELETE` API requests take path and/or query parameters. Meanwhile, `POST` API request accepts JSON body. Please note that there might be some exceptions which will be informed in the API doc and reference.
