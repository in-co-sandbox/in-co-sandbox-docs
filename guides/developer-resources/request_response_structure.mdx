...
description: "Learn about request formats, response structures, and HTTP headers used in Sandbox APIs."
keywords: ["request", "response", "headers", "JSON", "API structure", "HTTP methods"]
---
...
const response = await fetch('https://api.sandbox.co.in/gst/compliance/public/gstin/search', {
  method: 'POST',
  headers: {
    'x-api-key': 'key_live_SIethxxxxxxxx',
    'authorization': 'eyJhbGcxxxxx.eyJhdJxxxx.Si29xxxxx',
    'x-api-version': '1.0',
...
response = requests.post(
    ...
    headers={
        'x-api-key': 'key_live_SIethxxxxxxxx',
        'authorization': 'eyJhbGcxxxxx.eyJadJxxxx.Si29xxxxx',
        'x-api-version': '1.0',
        'Content-Type': 'application/json'
    },
    json={'gstin': '05ABNTY3290P8ZB'}
)
data = response.json()
```
</CodeGroup>
