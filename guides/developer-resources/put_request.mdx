...
<ParamField path="status" type="string">
  ...
  <Step title="Poll the job status">
    After uploading the input payload, the system processes the job asynchronously. Poll the same endpoint using a `GET` request with the `job_id` to track progress.
