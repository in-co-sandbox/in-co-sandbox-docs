...
<AccordionGroup>
  ...
  <Accordion title="What inputs are required for Track GST Returns?">
    Track GST Returns requires GSTIN in the request body and `financial_year` as a query parameter. Add the optional `gstr` query parameter to filter by return type. See [Errors](/guides/developer-resources/errors) for troubleshooting.
  </Accordion>
  ...
  <Accordion title="What is x-accept-cache and when should I use it?">
    It's an optional header that returns cached responses when available. This speeds up repeated requests. Set it to true if your workflow can tolerate cached data. See endpoint docs for [Search GSTIN](/api-reference/gst/compliance/endpoints/public/search_gstin) and [Track GST Returns](/api-reference/gst/compliance/endpoints/public/track_gstrs).
  </Accordion>
</AccordionGroup>
