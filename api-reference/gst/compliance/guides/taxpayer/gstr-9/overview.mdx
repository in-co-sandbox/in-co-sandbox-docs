...
GSTR-9 is the GST annual return filed once per financial year for a GSTIN. It reports consolidated purchases, sales, input tax credit (ITC), and tax paid. You can file GSTR-9 programmatically by fetching GSTN prepared summaries, reviewing Section 8A ITC, saving a draft, proceeding to filing, generating an EVC OTP, submitting the return, and tracking filing status on the GST portal.

## Key capabilities

<CardGroup cols={2}>
  <Card title="Auto-calculated annual totals" icon="database">
    Fetch GSTN prepared annual summaries to use as a reconciliation baseline for purchases, sales, and tax liability.
  </Card>
  ...
  <Card title="Draft save and proceed workflow" icon="paper-plane">
    Save prepared values as a draft on GSTN, then proceed to run portal validations before filing.
  </Card>
...
<Steps>
  ...
  <Step title="Prepare HSN mapping (if needed)">
    Use [Get HSN](/api-reference/gst/compliance/endpoints/taxpayer/gstr-1/documents/get_hsn) to build and validate HSN/SAC-level summaries when required for annual reporting.
  </Step>
  ...
  <Step title="Proceed to filing">
    Run GSTN validations using [Proceed GSTR-9](/api-reference/gst/compliance/endpoints/taxpayer/gstr-9/proceed).
  </Step>
...
<CardGroup cols={2}>
  <Card title="Draft and summaries" icon="database" href="/api-reference/gst/compliance/endpoints/taxpayer/gstr-9/gstr_9_details">
    Fetch the current GSTR-9 draft and auto-calculated annual totals for reconciliation.
...
<AccordionGroup>
  <Accordion title="What is GSTR-9 and who needs to file it?">
    GSTR-9 is the GST annual return that consolidates purchases, sales, input tax credit, and tax paid for a financial year. Regular taxpayers registered under GST must file GSTR-9 annually, except those exempted by government notification. Some taxpayer categories have different annual return requirements or may be exempt. Filing is required once all periodic returns for the FY are completed. See [File GSTR-9](/recipes/gst/gstr-9/file_gstr_9) for the API workflow.
  </Accordion>
  <Accordion title="How does the GSTR-9 API filing flow work end-to-end?">
    Start by initiating a taxpayer session for the GSTIN. Fetch the current draft and auto-calculated annual totals from GSTN. Review Section 8A ITC details and prepare HSN mappings if needed. Save the draft with [Save GSTR-9](/api-reference/gst/compliance/endpoints/taxpayer/gstr-9/save), then proceed to run validations. Generate an EVC OTP with [Generate EVC OTP](/api-reference/gst/compliance/endpoints/taxpayer/authentication/generate_evc_otp), file the return, and poll [GST return status](/api-reference/gst/compliance/endpoints/taxpayer/common/gst_return_status) until confirmation.
  </Accordion>
  ...
  <Accordion title="What do I do if filing is submitted but not showing as filed?">
    GSTR-9 filing can take time to reflect on the GST portal due to GSTN processing delays. Poll [Get GST return status](/api-reference/gst/compliance/endpoints/taxpayer/common/gst_return_status) until the status confirms filing completion. If the status stays pending beyond expected timeframes, check for GSTN errors in the response and consult the [error handling guide](/guides/developer-resources/errors).
  </Accordion>
