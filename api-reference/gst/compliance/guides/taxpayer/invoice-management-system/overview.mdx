---
title: "Overview"
description: "Automate GST invoice management with IMS APIs to fetch invoice counts, save accept/reject/pending actions, track processing status, and monitor recipient actions via Supplier View for Input Tax Credit compliance."
---
...
Supplier View shows suppliers the status of actions taken by recipients (buyers) on invoices reported via GSTR-1, GSTR-1A, or IFF. In simple terms: **"Did my customer accept, reject, or keep my invoice pending?"**
...
<AccordionGroup>
  ...
  <Accordion title="What do the recipient action statuses mean in Supplier View?">
    **Accepted**: Recipient has accepted the invoice in IMS. **Rejected**: Recipient has rejected the invoice due to mismatch or incorrect details. **Pending**: Recipient has kept it pending for later decision. **No Action**: Recipient has not acted yet, or the record is not actionable in IMS for certain cases like RCM or ineligible ITC.
  </Accordion>
  <Accordion title="What should I do if a recipient rejects my invoice?">
    If a recipient rejects your invoice, review the invoice details and correct any errors or mismatches. You can amend the invoice in GSTR-1A or the next relevant return filing. Use Supplier View to monitor which invoices are rejected and prioritize follow-up for high-value transactions.
  </Accordion>
