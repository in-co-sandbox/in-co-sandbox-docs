...
When you download the document file from the `url` returned in the response, you receive a signed XML certificate from DigiLocker. The XML structure varies by document type. Below are sample XML responses for commonly fetched documents.

<AccordionGroup>
  <Accordion title="Aadhaar (ADHAR)">
    The Aadhaar XML contains identity and address details within a `KycRes` envelope. The `UidData` element includes `Poi` (Proof of Identity), `Poa` (Proof of Address), `LData` (local language data), and `Pht` (base64-encoded photo).
  ...
  <Accordion title="PAN (PANCR)">
    The PAN XML follows a standard DigiLocker `Certificate` structure with `IssuedBy`, `IssuedTo`, and `CertificateData` sections.
  ...
  <Accordion title="Driving License (DRVLC)">
    The Driving License XML includes license details, vehicle categories, and both present and permanent addresses.
...
<Note>
  All XML responses are digitally signed by **Digital India Corporation** using an enveloped XML signature (`xmldsig#`). You can verify the document authenticity by validating the `<Signature>` block against the X.509 certificate included in the XML.
</Note>