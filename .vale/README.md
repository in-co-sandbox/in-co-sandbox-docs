# Vale Configuration for Sandbox API Docs

This directory contains the Vale linting configuration for Sandbox API documentation.

## Philosophy

Start simple and grow incrementally as needs emerge. The current setup uses:

- Core Sandbox-specific vocabulary
- Customized rules from the Google developer documentation style guide

## Vale files

- `.vale.ini` - Main configuration file
- `styles/config/vocabularies/Sandbox/` - Sandbox-specific terms
- `styles/Google/` - Google developer documentation style rules

## When to add vocabulary

- **Domain terms** - Tax, KYC, fintech-specific terms that trigger false positives
- **Product terms** - New components, features, platform-specific concepts
...
.vale/scripts/discover-vocabulary.sh "api-reference/*.mdx"
```
...
2. Run on sample files: `vale api-reference/*.mdx`
